...
from application.core.interfaces.adapter import AbstractDbAdapter
from application.infrastructure.database.models import Service, User


class DbAdapter(AbstractDbAdapter, ABC):
    ...
    async def create_subscription(
        ...
        return await self.session.execute(
            insert(Service).values(
                title=title, months=months, reminder=reminder, service_fk=service_fk,
            )
        )
    ...
    async def get_quantity_subs(self, user_id: int) -> None:
        return await self.session.scalar(
            select(User.count_subs).where(User.user_id == user_id)
        )

    async def get_language(self, user_id: int) -> None:
        return await self.session.scalar(
    ...
    async def increment_quantity(self, user_id: int) -> User:
        return await self.session.merge(
            User(user_id=user_id, count_subs=User.count_subs + 1)
        )

    async def decrement_quantity(self, user_id: int) -> User:
        return await self.session.merge(
            User(user_id=user_id, count_subs=User.count_subs - 1)
        )
