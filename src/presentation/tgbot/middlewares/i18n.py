...
from src.infrastructure.database.adapter.adapter import DbAdapter
from src.presentation.tgbot.constants import DEFAULT_LOCALE, LOCALES
...
def make_i18n_middleware():
    loader = FluentResourceLoader(
        os.path.join(
            os.path.dirname(__file__), '../../', 'locales', '{locale}'
        )
    )
    l10ns = {
