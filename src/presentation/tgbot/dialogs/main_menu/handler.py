...
from src.presentation.tgbot.dialogs.extras.i18n_format import I18N_FORMAT_KEY
from src.presentation.tgbot.states.user import MainMenu, CreateMenu, DeleteMenu
...
async def on_click_sub_create(
        callback: CallbackQuery, protocol: DialogProtocol, dialog_manager: DialogManager
) -> None:
    l10n = dialog_manager.middleware_data['l10n']
    session = dialog_manager.middleware_data['session']

    count_subs = await session.get_quantity_subs(user_id=dialog_manager.event.from_user.id)
...
async def on_click_sub_delete(
    ...
    await session.decrement_quantity(user_id=dialog_manager.event.from_user.id)
    await session.commit()

    await callback.message.edit_text(l10n.format_value('approve-sub-delete'))
    await dialog_manager.done()
    await dialog_manager.start(state=DeleteMenu.DELETE, mode=StartMode.RESET_STACK)
